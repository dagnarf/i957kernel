...
extern int qdsp_clock_register_read(void);
...
static int apr_modem_up = 0;
static int apr_q6_up = 0;

static ssize_t apr_modem_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", apr_modem_up);
}

static DEVICE_ATTR(apr_modem, S_IRUSR | S_IRGRP, apr_modem_show, NULL);

static ssize_t apr_q6_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", apr_q6_up);
}

static DEVICE_ATTR(apr_q6, S_IRUSR | S_IRGRP, apr_q6_show, NULL);
...
static int __init apr_tal_init(void)
{
	...
	apr_dev = device_create(sec_class, NULL, 0, NULL, "apr_dev");
	if (IS_ERR(apr_dev)) {
		printk(KERN_ERR "failed to create device!\n");
		return -1;
	}

	if (device_create_file(apr_dev, &dev_attr_apr_modem) < 0)
		pr_err("failed to create device file - %s\n", dev_attr_apr_modem.attr.name);

	if (device_create_file(apr_dev, &dev_attr_apr_q6) < 0)
		pr_err("failed to create device file - %s\n", dev_attr_apr_q6.attr.name);
