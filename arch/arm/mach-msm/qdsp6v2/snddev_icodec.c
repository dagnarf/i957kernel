...
extern struct adie_codec_action_unit headset_voip2_rx_legacy_48KHz_osr256_actions[];
...
#if defined(CONFIG_MACH_P5_LTE) && defined(CONFIG_TARGET_LOCALE_KOR_LGU)
	if(system_rev != 0x5) {
		pr_info("[QTR] P5_LTE_LGU HPH_CAPLESS_MODE (system_rev:%d) \n", system_rev);
	} else {
		// for rev03 only
		...
		if(strcmp(pdata->name, "headset_voip2_rx")==0) {
			pr_info("[QTR] %s name: %s\n", __func__, pdata->name); 
			pdata->profile->settings->actions = &headset_voip2_rx_legacy_48KHz_osr256_actions[0];
		}
