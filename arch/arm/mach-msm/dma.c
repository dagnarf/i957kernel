...
static int check_crci_conflict(struct msm_dmov_cmd *cmd, int adm)
{
	...
	for (i = 0; i < MSM_DMOV_CHANNEL_COUNT; i++) {
		if(dmov_conf[adm].active_commands[i].next->next == LIST_POISON1) {
			/* initalize the node to prevent crashed linked list */
			dmov_conf[adm].active_commands[i].next = &dmov_conf[adm].active_commands[i];
			dmov_conf[adm].active_commands[i].prev = &dmov_conf[adm].active_commands[i];
		}
		else {
