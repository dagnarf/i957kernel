...
static ssize_t
socinfo_show_board_revision(struct sys_device *dev,
			 struct sysdev_attribute *attr,
			 char *buf)
{
	if (!socinfo) {
...
#ifdef CONFIG_MACH_SAMSUNG_TARGET
static struct sysdev_attribute board_rev[] = {
	_SYSDEV_ATTR(board_revision, 0444, socinfo_show_board_revision, NULL),
};
#endif
...
#ifdef CONFIG_MACH_SAMSUNG_TARGET
	err = socinfo_create_files(&soc_sys_device, board_rev,
				ARRAY_SIZE(board_rev));
#endif
	return err;
}
