...
static void send_q6_nmi(void)
{
	...
	/* Q6 requires atleast 10ms to dump caches etc.*/
	mdelay(100);
...
static void __iomem *hwio_modem_reset_addr;
...
static int __init subsystem_fatal_init(void)
{
	...
	hwio_modem_reset_addr = ioremap_nocache(MODEM_HWIO_MSS_RESET_ADDR, 8);

	if (!hwio_modem_reset_addr)
		pr_err("%s: Unable to request modem reset interrupt.", __func__);
