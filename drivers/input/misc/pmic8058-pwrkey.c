...
static DEVICE_ATTR(pwrkey_pressed, S_IWUSR|S_IRUGO, keyshort_test, NULL);

static struct attribute *p5lteKeypad_attributes[] = {
	&dev_attr_pwrkey_pressed.attr,
	NULL,
};
...
static enum hrtimer_restart pmic8058_pwrkey_timer(struct hrtimer *timer)
{
	...
	input_report_key(pwrkey->pwr, KEY_POWER, 1);	//KEY_POWER
...
static irqreturn_t pwrkey_press_irq(int irq, void *_pwrkey)
{
	...
	if (pwrkey->pressed_first) {
		...
		sec_debug_check_crash_key(KEY_END, 1);
#endif
#endif
...
static irqreturn_t pwrkey_release_irq(int irq, void *_pwrkey)
{
	...
	if (pwrkey->pressed_first) {
		...
		if (!pwrkey->pdata->pwrkey_time_ms) {
			input_report_key(pwrkey->pwr, KEY_POWER, 0);	//KEY_POWER
			input_sync(pwrkey->pwr);
			spin_unlock_irqrestore(&pwrkey->lock, flags);
			return IRQ_HANDLED;
		}
...
#if defined(CONFIG_SEC_DEBUG)
		sec_debug_check_crash_key(KEY_END, 0);
#endif
#endif
	} else {
		/*
...
static int __devinit pmic8058_pwrkey_probe(struct platform_device *pdev)
{
	...
	keypad_dev = device_create(sec_class, NULL, 0, pwrkey, "power_key");
	if (IS_ERR(keypad_dev))
		pr_err("Failed to create device for the sysfs\n");

	err = sysfs_create_group(&keypad_dev->kobj, &p5ltekeypad_attr_group);
	if (err)
