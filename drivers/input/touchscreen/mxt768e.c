...
static void get_user_data(struct mxt540e_data *data)
{
	...
	read_mem(data, t38_address + 0, 1, &mxt768e_dbg.config_init_disable);
	read_mem(data, t38_address + 1, 1, &mxt768e_dbg.ta_probe_disable);
	read_mem(data, t38_address + 2, 1, &mxt768e_dbg.t55_ctrl_batt);
	read_mem(data, t38_address + 3, 1, &mxt768e_dbg.t55_thr_batt);
	read_mem(data, t38_address + 4, 1, &mxt768e_dbg.t57_thr_batt);
	read_mem(data, t38_address + 5, 1, &mxt768e_dbg.t55_ctrl_ta);
	read_mem(data, t38_address + 6, 1, &mxt768e_dbg.t55_thr_ta);
	read_mem(data, t38_address + 7, 1, &mxt768e_dbg.t57_thr_ta);
	read_mem(data, t38_address + 8, 1, &mxt768e_dbg.t8_chrtime_batt);
	read_mem(data, t38_address + 9, 1, &mxt768e_dbg.t8_chrtime_ta);
}
...
static void mxt_ta_probe(int ta_status)
{
	...
	if (mxt768e_dbg.ta_probe_disable == 1)
	{
		printk("[TSP] T38 mxt540e_ta_probe Disable: %d", mxt768e_dbg.ta_probe_disable);
		return;
	}
#endif
...
#else
			value = mxt768e_dbg.t8_chrtime_ta;
#endif
...
#else
			value = mxt768e_dbg.t8_chrtime_batt;
#endif
			error = write_mem(data, obj_address, 1, &value);
...
static u8 firmware_latest[] ={0x16, 0x12}; /* mxt224 : 0x16, mxt224E : 0x10 */
static u8 build_latest[] = {0xAB, 0xF2};
...
static int __devinit mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	copy_data = data;
#if 1 //defined(CONFIG_MACH_P8_REV01) || defined(CONFIG_MACH_P8LTE_REV00)
	if (client->addr == MXT768E_APP_LOW)
		client->addr = MXT768E_BOOT_LOW;
	else
		client->addr = MXT768E_BOOT_HIGH;
...
#if 0            
			if (mxt_load_fw(NULL, MXT768E_FW_NAME)) goto err_config;
			else {
#endif                
				msleep(MXT_RESET_TIME);
				mxt_init_touch_driver(data);
