...
#define SUPPORT_5X2_MATRIX //Qualcomm patch for keypad matrix 2*5 
#endif
...
static int pmic8058_kp_use_standalone_isr(void)
{
...
#ifdef CONFIG_KEYPAD_P5LTE
/* stealth - volumeup , volumedown */
static int gpio_key_scan(struct pmic8058_kp *kp, int irq)
{
	int state=0, code=0;
	
	if (irq == kp->irq_volume_down){
		code = kp->code_volume_down;
		state = gpio_get_value_cansleep(kp->key_volume_down);
	}else {	
		code = kp->code_volume_up;
		state = gpio_get_value_cansleep(kp->key_volume_up);
	}
	 
	input_event(kp->input, EV_MSC, MSC_SCAN, code );
	input_report_key(kp->input, code, state);
	input_sync(kp->input);

	printk("key [%s]\n", state ? "pressed" : "released"); 
//	printk("key [keycode: %d, %s]\n", code, state ? "pressed" : "released"); 
...
static irqreturn_t pmic8058_kp_irq(int irq, void *data)
{
	...
	if (pmic8058_kp_use_standalone_isr()) {
		mdelay(1);
		if ( irq == kp->irq_volume_up || irq == kp->irq_volume_down){ 
			mdelay(1);
			gpio_key_scan(kp, irq);
		}
	} else {
		if (pm8058_rev(kp->pm_chip) == PM_8058_REV_1p0)
		mdelay(1);
...
static ssize_t keyshort_test(struct device *dev, struct device_attribute *attr,	char *buf)
{
	...
	int row, col;
	struct pmic8058_kp *kp = dev_get_drvdata(dev);
//	printk("[KEY]GPIOKEY_PRESSED key_vol_up:%d, code_vol_up:%d, irq_vol_up:%d, get_key:%d \n", 
//				kp->key_volume_up, kp->code_volume_up, kp->irq_volume_up,
//				gpio_get_value_cansleep(kp->key_volume_up));
#if 0
	if (gpio_get_value_cansleep(kp->key_volume_down)||gpio_get_value_cansleep(kp->key_volume_up))
			keycount++;
#endif
	for(row=0; row<kp->pdata->num_rows; row++){
		for(col=0; col<kp->pdata->num_cols; col++){
...
#ifdef CONFIG_KEYPAD_P5LTE
static DEVICE_ATTR(gpiokey_pressed, S_IWUSR|S_IRUGO, keyshort_test , NULL);

static struct attribute *p5lteKeypad_attributes[] = {
	&dev_attr_gpiokey_pressed.attr,
	NULL,
};

static struct attribute_group p5ltekeypad_attr_group = {
	.attrs = p5lteKeypad_attributes,
};
#endif
...
static int __devinit pmic8058_kp_probe(struct platform_device *pdev)
{
	...
	if (pmic8058_kp_use_standalone_isr()) {
		kp->key_volume_down = platform_get_irq(pdev, 0);
		if (kp->key_volume_down < 0) {
			dev_err(&pdev->dev, "unable to get volume down irq\n");
			rc = -ENXIO;
			goto err_get_irq;
		}
		kp->irq_volume_down = gpio_to_irq(kp->key_volume_down);
		pr_info("key down_irq: 0x%02x\n", kp->irq_volume_down);
		kp->code_volume_down = KEY_VOLUMEDOWN; 

		kp->key_volume_up = platform_get_irq(pdev, 1);
		if (kp->key_volume_up < 0) {
			dev_err(&pdev->dev, "unable to get volume up irq\n");
			rc = -ENXIO;
			goto err_get_irq;
		}
		kp->irq_volume_up = gpio_to_irq(kp->key_volume_up);
		pr_info("key up_irq: 0x%02x\n", kp->irq_volume_up);
		kp->code_volume_up = KEY_VOLUMEUP;
	}else {
	...
	if (pmic8058_kp_use_standalone_isr()) {
		kp->pdata->init_key_gpio();

		rc = request_threaded_irq(kp->irq_volume_down, NULL, pmic8058_kp_irq, 
					IRQF_TRIGGER_RISING |IRQF_TRIGGER_FALLING, "vol_down", kp);
			if (rc < 0) {
				dev_err(&pdev->dev, "failed to request vol_down irq\n");
				goto err_req_voldown_irq;
			}

		rc = request_threaded_irq(kp->irq_volume_up, NULL, pmic8058_kp_irq, 
					IRQF_TRIGGER_RISING |IRQF_TRIGGER_FALLING, "vol_up", kp);
		if (rc < 0) {
			dev_err(&pdev->dev, "failed to request vol_up irq\n");
			goto err_req_volup_irq;
		}
	...
	keypad_dev = device_create(sec_class, NULL, 0, kp, "gpio_keys");
	if (IS_ERR(keypad_dev))
		pr_err("Failed to create device for the sysfs\n");

	rc = sysfs_create_group(&keypad_dev->kobj, &p5ltekeypad_attr_group);
	if (rc)
...
err_req_sense_irq:
	if (pmic8058_kp_use_standalone_isr()) {
		free_irq(kp->irq_volume_up, kp);
		free_irq(kp->irq_volume_down, kp);
	} else {
