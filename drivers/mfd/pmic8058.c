...
#include "../../arch/arm/mach-msm/clock-8x60.h"
...
ssbi_write(struct i2c_client *client, u16 addr, const u8 *buf, size_t len)
{
	...
	if (rc != 1) {
		pr_err("%s: ERROR: PM8058 ssbi write failed, rc=%d\n",
			__func__, rc);
		dump_ssbi_registers();
	}
	return (rc == 1) ? 0 : rc;
...
ssbi_read(struct i2c_client *client, u16 addr, u8 *buf, size_t len)
{
	...
	rc = i2c_transfer(client->adapter, &msg, 1);
	if (rc != 1) {
		pr_err("%s: ERROR: PM8058 ssbi read failed, rc=%d\n",
			__func__, rc);
		dump_ssbi_registers();
	}
	return (rc == 1) ? 0 : rc;
...
#define MPM_CXO_CNTRL 0x14
#define MPM_SLEEP_TIMETICK_COUNT_VAL 0x24
#define MPM_LOW_PWR_CONFIG 0x28
#define MPM_LOW_PWR_STATUS 0x2C
void mpm_regs_dump(void)
{
	pr_err("%s: MPM_CXO_CNTRL:  0x%08X\n", __func__,
	    readl_relaxed(MSM_MPM_BASE + MPM_CXO_CNTRL));
	pr_err("%s: MPM_SLEEP_TIMETICK_COUNT_VAL:  0x%08X\n", __func__,
	    readl_relaxed(MSM_MPM_BASE + MPM_SLEEP_TIMETICK_COUNT_VAL));
	pr_err("%s: MPM_LOW_PWR_CONFIG:  0x%08X\n", __func__,
	    readl_relaxed(MSM_MPM_BASE + MPM_LOW_PWR_CONFIG));
	pr_err("%s: MPM_LOW_PWR_STATUS:  0x%08X\n", __func__,
	    readl_relaxed(MSM_MPM_BASE + MPM_LOW_PWR_STATUS));
}

#define OFFSET_RPM_CLK_BRANCH_ENA_VOTE		0x2FE0
#define OFFSET_MARM_CLK_BRANCH_ENA_VOTE		0x3000
#define OFFSET_LPA_CLK_BRANCH_ENA_VOTE		0x3020
#define OFFSET_SC0_U_CLK_BRANCH_ENA_VOTE	0x3080
#define OFFSET_SC1_U_CLK_BRANCH_ENA_VOTE	0x30A0
#define OFFSET_CLK_HALT_SFPB_MISC_STATE		0x2FD8

void dump_ssbi_registers(void)
{
	unsigned long flag;

	pr_err("%s: RPM_CLK_BRANCH_ENA_VOTE:   0x%08X\n", __func__,
	    readl_relaxed(OFFSET_RPM_CLK_BRANCH_ENA_VOTE + MSM_CLK_CTL_BASE));
	pr_err("%s: MARM_CLK_BRANCH_ENA_VOTE:  0x%08X\n", __func__,
	    readl_relaxed(OFFSET_MARM_CLK_BRANCH_ENA_VOTE + MSM_CLK_CTL_BASE));
	pr_err("%s: LPA_CLK_BRANCH_ENA_VOTE:   0x%08X\n", __func__,
	    readl_relaxed(OFFSET_LPA_CLK_BRANCH_ENA_VOTE + MSM_CLK_CTL_BASE));
	pr_err("%s: SC0_U_CLK_BRANCH_ENA_VOTE: 0x%08X\n", __func__,
	    readl_relaxed(OFFSET_SC0_U_CLK_BRANCH_ENA_VOTE + MSM_CLK_CTL_BASE));
	pr_err("%s: SC1_U_CLK_BRANCH_ENA_VOTE: 0x%08X\n", __func__,
	    readl_relaxed(OFFSET_SC1_U_CLK_BRANCH_ENA_VOTE + MSM_CLK_CTL_BASE));
	pr_err("%s: CLK_HALT_SFPB_MISC_STATE:  0x%08X\n", __func__,
	    readl_relaxed(OFFSET_CLK_HALT_SFPB_MISC_STATE + MSM_CLK_CTL_BASE));

	mpm_regs_dump();

	/* measure the cxo clock rate, the pmic wont respond if the cxo clock is off */
	pr_info("PMIC_SSBI2_CLK is at %d Hz\n",
		soc_clk_measure_rate(L_PMIC_SSBI2_CLK));
