...
#ifdef CONFIG_MACH_SAMSUNG_TARGET
	rc = pm8058_rtc0_read_time(&pdev->dev, &tm);
	if (rc) {
		pr_err("%s: Unable to read RTC time\n", __func__);
	}
	rtc_tm_to_time(&tm, &tv_sec);

	if ( tm.tm_year + 1900 < 2011 ) {
		pr_debug("rtc year is below 2011. initialize to 2011-01-01\n");
...
#ifdef CONFIG_RTC_PM8058_WRITE_ENABLE
		rc = pm8058_rtc0_set_time(&pdev->dev, &tm);
		if (rc) {
