...
#define XOADC_REF_VOL_MIN 2100000 /*uV*/
#define XOADC_REF_VOL_MAX 2300000 /*uV*/
...
static int __devinit pmic8058_tm_probe(struct platform_device *pdev)
{
	...
	ldo18 = regulator_get(NULL, "8058_l18");
	if( IS_ERR(ldo18) )
		return PTR_ERR(ldo18);
	rc = regulator_set_voltage(ldo18, XOADC_REF_VOL_MIN, XOADC_REF_VOL_MAX );
	if( rc  ) {
		printk("%s: error setting 8058_l18 voltage\n", __func__);
		return 0;
	}	
