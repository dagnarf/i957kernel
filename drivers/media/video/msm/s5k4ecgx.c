...
static int s5k4ecgx_sensor_burst_write(const u32 *list, int size, char *name)
{
	int i = 0;
	int idx = 0;
	int err = -EINVAL;
...
#else	// for Test (Jeonhk 20110328)
	
	for(i =0; i < size; i++) {
		subaddr = ((list[i].value)>> 16); //address	
		value = ((list[i].value)& 0xFFFF); //value
		//printk("subaddr = 0x%x, value = 0x%x\n",subaddr,value);
		if((err = s5k4ecgx_sensor_write(subaddr,value)) < 0) {
			printk("[S5K4ECGX]%s: register set failed. try again.\n",__func__);
		}
	}		
#endif	
...
void s5k4ecgx_set_metering(char value)
{
	...
	switch(value)
	{
		case PCAM_METERING_NORMAL :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Metering_Matrix_EVT1);
		break;
		case PCAM_METERING_SPOT :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Metering_Spot_EVT1);
		break;
		case PCAM_METERING_CENTER :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Metering_Center_EVT1);
		break;
		default :
...
void s5k4ecgx_set_contrast(char value)
{
	...
	switch(value)
	{
		case PCAM_CR_STEP_M_2 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Contrast_Minus_2_EVT1);
		break;
		case PCAM_CR_STEP_M_1 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Contrast_Minus_1_EVT1);
		break;
		case PCAM_CR_STEP_0 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Contrast_Default_EVT1);
		break;
		case PCAM_CR_STEP_P_1 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Contrast_Plus_1_EVT1);
		break;
		case PCAM_CR_STEP_P_2 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Contrast_Plus_2_EVT1);
		break;
		default :
...
void s5k4ecgx_set_zoom(char value)
{
	...
	switch(value)
	{
		case PCAM_ZOOM_STEP_0 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_0_EVT1);
		break;
...
void s5k4ecgx_set_saturation(char value)
{
	...
	switch(value)
	{
		case PCAM_SA_STEP_M_2 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Saturation_Minus_2_EVT1);
		break;
		case PCAM_SA_STEP_M_1 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Saturation_Minus_1_EVT1);
		break;
		case PCAM_SA_STEP_0 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Saturation_Default_EVT1);
		break;
		case PCAM_SA_STEP_P_1 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Saturation_Plus_1_EVT1);
		break;
		case PCAM_SA_STEP_P_2 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Saturation_Plus_2_EVT1);
		break;
		default :
...
void s5k4ecgx_set_sharpness(char value)
{
	...
	switch(value)
	{
		case PCAM_SP_STEP_M_2 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Sharpness_Minus_2_EVT1);
		break;
		case PCAM_SP_STEP_M_1 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Sharpness_Minus_1_EVT1);
		break;
		case PCAM_SP_STEP_0 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Sharpness_Default_EVT1);
		break;
		case PCAM_SP_STEP_P_1 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Sharpness_Plus_1_EVT1);
		break;
		case PCAM_SP_STEP_P_2 :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Sharpness_Plus_2_EVT1);
		break;
		default :
...
void s5k4ecgx_set_auto_contrast(char value)
{
	...
	switch(value)
	{
		case PCAM_AUTO_CONTRAST_ON :
			...
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Auto_Contrast_ON_EVT1);
		break;
		case PCAM_AUTO_CONTRAST_OFF :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Auto_Contrast_OFF_EVT1);
...
void s5k4ecgx_set_jpeg_quality(char value)
{
	...
	switch(value)
	{
		case PCAM_JPEG_QUALITY_SUPERFINE :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Jpeg_Quality_High_EVT1);
		break;
		case PCAM_JPEG_QUALITY_FINE :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Jpeg_Quality_Normal_EVT1);
		break;
		case PCAM_JPEG_QUALITY_NORMAL :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Jpeg_Quality_Low_EVT1);
		break;
		default :
...
void s5k4ecgx_set_DTP(char value)
{
	...
	{
		case PCAM_DTP_OFF:
			S5K4ECGX_WRITE_LIST(s5k4ecgx_DTP_stop_EVT1);
		break;
		case PCAM_DTP_ON:
			S5K4ECGX_WRITE_LIST(s5k4ecgx_DTP_init_EVT1);
		break;
		default:
...
void s5k4ecgx_set_aeawe_lock(char value)
{
	...
	switch(value)
	{
		case PCAM_AWB_AE_LOCK :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_awb_ae_lock_EVT1 );
		break;
		case PCAM_AWB_AE_UNLOCK :
			S5K4ECGX_WRITE_LIST(s5k4ecgx_awb_ae_unlock_EVT1);
		break;
		default :
...
void s5k4ecgx_set_capture_size(int size)
{
	...
	else if(size == PCAM_SNAPSHOT_SIZE_2048x1536_3M)S5K4ECGX_WRITE_LIST(s5k4ecgx_3M_Capture_EVT1)
	else if(size == PCAM_SNAPSHOT_SIZE_1600x1200_2M)S5K4ECGX_WRITE_LIST(s5k4ecgx_2M_Capture_EVT1)
	else if(size == PCAM_SNAPSHOT_SIZE_1280x960_1M)S5K4ECGX_WRITE_LIST(s5k4ecgx_1M_Capture_EVT1)
	else if(size == PCAM_SNAPSHOT_SIZE_640x480_VGA)S5K4ECGX_WRITE_LIST(s5k4ecgx_VGA_Capture_EVT1)
	else if(size == PCAM_SNAPSHOT_SIZE_320x240_QVGA)S5K4ECGX_WRITE_LIST(s5k4ecgx_QVGA_Capture_EVT1)
...
#if 0
	if(size == PCAM_PREVIEW_SIZE_640x480_VGA)S5K4ECGX_WRITE_LIST(s5k4ecgx_640_Preview_EVT1)
	else if(size == PCAM_PREVIEW_SIZE_176x144_QCIF)S5K4ECGX_WRITE_LIST(s5k4ecgx_640_Preview_EVT1)
	else if(size == PCAM_PREVIEW_SIZE_320x240_QVGA)S5K4ECGX_WRITE_LIST(s5k4ecgx_176_Preview_EVT1)
	else if(size == PCAM_PREVIEW_SIZE_720x480_D1)S5K4ECGX_WRITE_LIST(s5k4ecgx_720_Preview_EVT1)
...
/*
	rc = s5k4ecgx_sensor_burst_write(s5k4ecgx_Preview_Return_EVT1,
			(sizeof(s5k4ecgx_Preview_Return_EVT1) / sizeof(s5k4ecgx_Preview_Return_EVT1[0])),
			"s5k4ecgx_Preview_Return_EVT1");
*/	
	rc = s5k4ecgx_sensor_burst_write(s5k4ecgx_640_Preview_EVT1,
			(sizeof(s5k4ecgx_640_Preview_EVT1) / sizeof(s5k4ecgx_640_Preview_EVT1[0])),
...
void s5k4ecgx_set_capture(void)
{
	...
	{
		S5K4ECGX_WRITE_LIST(s5k4ecgx_Capture_Start_EVT1);
//		mdelay(80);		
	...
	else /*low light condition */
	{
		...
		{
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Night_Mode_On_EVT1);
			mdelay(250);
...
//			mdelay(300);			
		...
		{
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Low_Cap_On_EVT1);
			mdelay(120);
			S5K4ECGX_WRITE_LIST(s5k4ecgx_Capture_Start_EVT1);
//			mdelay(80);			
			s5k4ecgx_check_REG_TC_GP_EnableCaptureChanged();
...
int s5k4ecgx_mipi_mode(int mode)
{
	int rc = 0;
...
static int s5k4ecgx_start(void)
{
	...
	PCAM_DEBUG("%s E\n",__FUNCTION__);
	PCAM_DEBUG("I2C address : 0x%x\n",s5k4ecgx_client->addr);
	
	if(s5k4ecgx_ctrl->started)
	{
		PCAM_DEBUG("%s X : already started\n",__FUNCTION__);
		return rc;
	}
	...
	/* enter read mode */
	s5k4ecgx_sensor_write( 0x002C, 0x7000);
	s5k4ecgx_sensor_write(0x002E, 0x01A4);
	
	s5k4ecgx_sensor_read(0x0F12, &read_value);	// 0x4EC0

	PCAM_DEBUG("%s : FW ID1  revision 0x%X\n", __func__, read_value);

	s5k4ecgx_sensor_write( 0x002C, 0x7000);
	s5k4ecgx_sensor_write( 0x002E, 0x01A6);		// 0x11
	
	s5k4ecgx_sensor_read( 0x0F12, &read_value);

	PCAM_DEBUG("%s :FW ID2 revision 0x%X\n", __func__, read_value);
	...
	rc = s5k4ecgx_sensor_burst_write(s5k4ecgx_init_reg2_EVT1,
			(sizeof(s5k4ecgx_init_reg2_EVT1) / sizeof(s5k4ecgx_init_reg2_EVT1[0])),
			"s5k4ecgx_init_reg2_EVT1");	
	msleep(10);	

	rc = s5k4ecgx_sensor_burst_write(s5k4ecgx_DTP_stop_EVT1,
			(sizeof(s5k4ecgx_DTP_stop_EVT1) / sizeof(s5k4ecgx_DTP_stop_EVT1[0])),
			"s5k4ecgx_DTP_stop_EVT1");
	msleep(10);

	s5k4ecgx_ctrl->started = 1;
}
...
static int s5k4ecgx_sensor_init_probe()
{
	...
	gpio_set_value_cansleep(CAM_5M_RST, LOW);
	gpio_set_value_cansleep(CAM_5M_STBY, LOW);	
	...
	gpio_set_value_cansleep(CAM_5M_STBY, HIGH);	
	udelay(20);
	gpio_set_value_cansleep(CAM_5M_RST, HIGH);
	...
	const struct i2c_device_id *id)
{
	...
	printk("[S5K4ECGX] s5k4ecgx_i2c_probe succeeded!\n");
