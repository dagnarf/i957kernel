...
int s5k5bafx_i2c_tx_data(char* txData, int length)
{
	int rc; 
	...
	if (rc < 0) {
		printk(KERN_ERR "s5k5bafx: s5k5bafx_i2c_tx_data error %d\n", rc);
		return rc;
	}
...
static int s5k5bafx_i2c_read(unsigned short page, unsigned short subaddr, unsigned short *data)
{
	int ret;
...
static int s5k5bafx_i2c_write(unsigned char u_addr, unsigned char u_data)
{
	...
	if (i2c_transfer(s5k5bafx_client->adapter, &msg, 1) < 0) {
		printk("s5k5bafx_i2c_write failed(addr : 0x%x , value : 0x%x\n",u_addr,u_data);
		return -EIO;
	}
	return 0;
}
...
static int32_t s5k5bafx_i2c_txdata(unsigned short saddr, unsigned long packet)
{
	unsigned char buf[4];
	int err = 0;
	...
	*(unsigned long *)buf = cpu_to_be32(packet);
	...
	if( i2c_transfer(s5k5bafx_client->adapter, &msg, 1) < 0)
	{
		printk("s5k5bafx_i2c_txdata failed\n");
		//return -EIO;
	}
#else
	i2c_transfer(s5k5bafx_client->adapter, &msg, 1);
...
static int32_t s5k5bafx_i2c_write_32bit(unsigned short saddr, unsigned long packet)
{
	...
	for(i=0; i< retry_count; i++) {
		rc = s5k5bafx_i2c_txdata(saddr, packet);

#if 0
		if(rc < 0) {
			mdelay(10);
			//printk("i2c_write failed(%d/%d), 0x%x\n", i, retry_count, packet);
		}
#endif	
	}
...
static int s5k5bafx_i2c_write_list(const unsigned long *list, int size, char *name)
{
	...
	for (i = 0; i < size; i++)
	{
		...
		//if(s5k5bafx_i2c_txdata(s5k5bafx_client->addr, temp_packet) < 0)
		if(s5k5bafx_i2c_write_32bit(s5k5bafx_client->addr, temp_packet) < 0)
		{
			printk("s5k5bafx_i2c_write_32bit fail(0x%x, 0x%x:%d)\n", s5k5bafx_client->addr, temp_packet, i);
			//return -1;
		}
		//udelay(10);
...
//static long s5k5bafx_set_sensor_mode(enum sensor_mode_t mode)
static long s5k5bafx_set_sensor_mode(int mode)
{
	...
	case SENSOR_PREVIEW_MODE:
		...
		s5k5bafx_start();
		s5k5bafx_i2c_write_list(s5k5bafx_preview, sizeof(s5k5bafx_preview) / sizeof(s5k5bafx_preview[0]), "s5k5bafx_preview");
	#if 0
		if(s5k5bafx_ctrl->dtp_mode == 1)
		{
			//s5k5bafx_i2c_write_list(reg_dtp_preview_camera_list,sizeof(reg_dtp_preview_camera_list)/sizeof(reg_dtp_preview_camera_list[0]),"reg_dtp_preview_camera_list");
		}
		else
		{
			if(s5k5bafx_ctrl->vtcall_mode)
			{
				s5k5bafx_i2c_write_list(vtmode_list,sizeof(vtmode_list)/sizeof(vtmode_list[0]),"vtmode_list");
			}
			else
			{
				s5k5bafx_i2c_read(0x00,0xE5,&value);
				CAM_DEBUG(" 0x00,0xE5 value : 0x%x\n",value);
				shade_value = value * 0x100;
				s5k5bafx_i2c_read(0x00,0xE6,&value);
				CAM_DEBUG(" 0x00,0xE6 value : 0x%x\n",value);
				shade_value += value;

				if((shade_value >= reg_AgcValue_list[3]) && (shade_value <= reg_AgcValue_list[4]))
				{
						s5k5bafx_i2c_write_list(reg_shade_dnp_list,sizeof(reg_shade_dnp_list)/sizeof(reg_shade_dnp_list[0]),"reg_shade_dnp_list");
				}
				else
				{
					s5k5bafx_i2c_write_list(reg_shade_etc_list,sizeof(reg_shade_etc_list)/sizeof(reg_shade_etc_list[0]),"reg_shade_etc_list");				
				}

				
				if(s5k5bafx_ctrl->app_mode == CAMCORDER_MODE) 
				{
					s5k5bafx_i2c_write_list(reg_preview_camcorder_list,sizeof(reg_preview_camcorder_list)/sizeof(reg_preview_camcorder_list[0]),"reg_preview_camcorder_list");
				}
				else		// camera mode preview
				{				
					s5k5bafx_i2c_write_list(reg_preview_camera_list,sizeof(reg_preview_camera_list)/sizeof(reg_preview_camera_list[0]),"reg_preview_camera_list");
					s5k5bafx_i2c_read(0x00,0xF8,&agc_value);
					if(agc_value <= reg_AgcValue_list[0])
					{
						s5k5bafx_i2c_write_list(reg_preview_high_light_list,sizeof(reg_preview_high_light_list)/sizeof(reg_preview_high_light_list[0]),"reg_preview_high_light_list");				
					}
					else
					{
						s5k5bafx_i2c_write_list(reg_preview_normal_light_list,sizeof(reg_preview_normal_light_list)/sizeof(reg_preview_normal_light_list[0]),"reg_preview_normal_light_list");				
					}
				}
			}
		}
	...
	case SENSOR_SNAPSHOT_MODE:
		...
		s5k5bafx_i2c_write_list(s5k5bafx_capture, sizeof(s5k5bafx_capture) / sizeof(s5k5bafx_capture[0]), "s5k5bafx_capture");
		...
		{
			s5k5bafx_i2c_write_list(reg_snapshot_lowlight_list,sizeof(reg_snapshot_lowlight_list)/sizeof(reg_snapshot_lowlight_list[0]),"reg_snapshot_lowlight_list");
		}
		else if(agc_value >= reg_AgcValue_list[1])
		{
			s5k5bafx_i2c_write_list(reg_snapshot_mid_lowlight_list,sizeof(reg_snapshot_mid_lowlight_list)/sizeof(reg_snapshot_mid_lowlight_list[0]),"reg_snapshot_mid_lowlight_list");
		}
		else if(agc_value <= reg_AgcValue_list[0])
		{
			s5k5bafx_i2c_write_list(reg_snapshot_list,sizeof(reg_snapshot_list)/sizeof(reg_snapshot_list[0]),"reg_snapshot_list");
		}
		else
		{
			s5k5bafx_i2c_write_list(reg_snapshot_midlight_list,sizeof(reg_snapshot_midlight_list)/sizeof(reg_snapshot_midlight_list[0]),"reg_snapshot_midlight_list");
		}
		//test
		#endif
...
static long s5k5bafx_set_effect(
	int mode,
	int8_t effect
)
{
...
static int s5k5bafx_reset(void)
{
	CAM_DEBUG("s5k5bafx_reset");
#if 0
	if (system_rev >= 4) 
	...
	int8_t ev)
{
	...
	switch(ev)
	{
		case S5K5BAFX_EV_MINUS_4:
			s5k5bafx_i2c_write_list(reg_brightness_0_list,sizeof(reg_brightness_0_list)/sizeof(reg_brightness_0_list[0]),"reg_brightness_0_list");
			break;
		case S5K5BAFX_EV_MINUS_3:
			s5k5bafx_i2c_write_list(reg_brightness_1_list,sizeof(reg_brightness_1_list)/sizeof(reg_brightness_1_list[0]),"reg_brightness_1_list");
			break;
		case S5K5BAFX_EV_MINUS_2:
			s5k5bafx_i2c_write_list(reg_brightness_2_list,sizeof(reg_brightness_2_list)/sizeof(reg_brightness_2_list[0]),"reg_brightness_2_list");
			break;
		case S5K5BAFX_EV_MINUS_1:
			s5k5bafx_i2c_write_list(reg_brightness_3_list,sizeof(reg_brightness_3_list)/sizeof(reg_brightness_3_list[0]),"reg_brightness_3_list");
			break;
		case S5K5BAFX_EV_DEFAULT:
			s5k5bafx_i2c_write_list(reg_brightness_4_list,sizeof(reg_brightness_4_list)/sizeof(reg_brightness_4_list[0]),"reg_brightness_4_list");
			break;
		case S5K5BAFX_EV_PLUS_1:
			s5k5bafx_i2c_write_list(reg_brightness_5_list,sizeof(reg_brightness_5_list)/sizeof(reg_brightness_5_list[0]),"reg_brightness_5_list");
			break;
		case S5K5BAFX_EV_PLUS_2:
			s5k5bafx_i2c_write_list(reg_brightness_6_list,sizeof(reg_brightness_6_list)/sizeof(reg_brightness_6_list[0]),"reg_brightness_6_list");
			break;	
		case S5K5BAFX_EV_PLUS_3:
			s5k5bafx_i2c_write_list(reg_brightness_7_list,sizeof(reg_brightness_7_list)/sizeof(reg_brightness_7_list[0]),"reg_brightness_7_list");
			break;
		case S5K5BAFX_EV_PLUS_4:
			s5k5bafx_i2c_write_list(reg_brightness_8_list,sizeof(reg_brightness_8_list)/sizeof(reg_brightness_8_list[0]),"reg_brightness_8_list");
			break;
		default:
	...
	unsigned int mode, unsigned int fps)
{
	...
	if(mode)
	{
		...
		s5k5bafx_ctrl->app_mode = CAMCORDER_MODE;
	}
	else
	{
		CAM_DEBUG("mode change to CAMERA_MODE");
		s5k5bafx_ctrl->app_mode = CAMERA_MODE;
	}
	return 0;
}
#if 0
...
#if 0 // for_8M_cam
static int s5k5bafx_init_regs(void)
{
	CAM_DEBUG("%s E\n",__FUNCTION__);
	return 0;
}
...
static int s5k5bafx_i2c_test(void)
{
	unsigned short value=0;
	CAM_DEBUG("%s E\n",__FUNCTION__);
	CAM_DEBUG("----- WRITE -----\n");
	s5k5bafx_i2c_write(0xfc,0x00);
	CAM_DEBUG("write : 0x02 - 0x08, 0x03 - 0x4b\n");
	s5k5bafx_i2c_write(0x02,0x08);
	s5k5bafx_i2c_write(0x03,0x4b);
	
	CAM_DEBUG("----- READ -----\n");
	s5k5bafx_i2c_read(0x00,0x02,&value);
	CAM_DEBUG(" 0x02, value : 0x%x\n",value);
	s5k5bafx_i2c_read(0x00,0x03,&value);
	CAM_DEBUG(" 0x03 value : 0x%x\n",value);
	s5k5bafx_i2c_read(0x01,0x01,&value);
...
static int s5k5bafx_start(void)
{
	...
	s5k5bafx_i2c_write_list(s5k5bafx_init,sizeof(s5k5bafx_init)/sizeof(s5k5bafx_init[0]),"s5k5bafx_init");
	//s5k5bafx_i2c_write_list(reg_effect_none_list,sizeof(reg_effect_none_list)/sizeof(reg_effect_none_list[0]),"reg_effect_none_list");
	//s5k5bafx_i2c_write_list(reg_meter_center_list,sizeof(reg_meter_center_list)/sizeof(reg_meter_center_list[0]),"reg_meter_center_list");
	//s5k5bafx_i2c_write_list(reg_wb_auto_list,sizeof(reg_wb_auto_list)/sizeof(reg_wb_auto_list[0]),"reg_wb_auto_list");
...
static int s5k5bafx_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
	...
	CAM_DEBUG("s5k5bafx_sensor_init_probe start");
	...
	//for test
	//s5k5bafx_i2c_txdata(s5k5bafx_client->addr, 0xFCFCD000);
	return rc;
}
...
int s5k5bafx_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	...
	if (!s5k5bafx_ctrl) {
		CDBG("s5k5bafx_sensor_open_init failed!\n");
		rc = -ENOMEM;
		goto init_done;
	}	
...
int s5k5bafx_sensor_ext_config(void __user *argp)
{
	...
	CAM_DEBUG("s5k5bafx_sensor_ext_config, cmd = %d , param1 = %d",cfg_data.cmd,cfg_data.value_1);
	
	switch(cfg_data.cmd) {
	case EXT_CFG_SET_BRIGHTNESS:
		rc = s5k5bafx_set_ev(cfg_data.value_1);
		break;
...
int s5k5bafx_sensor_config(void __user *argp)
{
	...
	CDBG("s5k5bafx_sensor_config, cfgtype = %d, mode = %d\n",
...
static int s5k5bafx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CDBG("s5k5bafx_i2c_probe successed!\n");
	CAM_DEBUG("%s X\n",__FUNCTION__);
...
probe_failure:
	...
	CDBG("s5k5bafx_i2c_probe failed!\n");
	CAM_DEBUG("s5k5bafx_i2c_probe failed!\n");
	return rc;
}
...
init_failure:
	CDBG("failed to s5k5bafx_i2c_init, rc = %d\n", rc);
	return rc;
}
