...
// for_8M_cam static struct vreg *vreg_CAM_AF28;
...
static int m5mo_mipi_mode(int mode)
{
	...
	if (!config_csi) {
		...
		m5mo_csi_params.settle_cnt = 24; //0x7; //0x14;
...
#if	0
	m5mo_i2c_write_8bit(0x01, 0x01, 0x17);
	m5mo_i2c_write_8bit(0x00, 0x10, 0x01);
	m5mo_i2c_write_8bit(0x00, 0x0B, 0x02);
	mdelay(60);

	/* Move the lens to infinity */
	m5mo_i2c_write_8bit(0x0a, 0x10, 0x01);
#endif	
	return rc;
}
...
//	m5mo_i2c_write_8bit(0x06, 0x00, 0x00);
	...
	/* MIPI mode  */
	m5mo_i2c_write_8bit(0x01, 0x01, 0x02);   //test
	...
	m5mo_i2c_write_32bit(0x0B, 0x3C, 0x10000);
#endif	
...
#if	0	// AP_JPEG
		//m5mo_i2c_write_8bit(0x0B, 0x00, 0x00);	//  YUV
#else
...
//	int fd=0;
	...
	CAM_DEBUG("3. CAM_VGA_RST HIGH ");
	gpio_set_value_cansleep(CAM_VGA_RST, HIGH);
	mdelay(2);
	
	CAM_DEBUG("4. CAM_VGA_STBY LOW ");
	gpio_set_value_cansleep(CAM_VGA_EN, LOW);
...
int m5mo_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	...
	if (!m5mo_ctrl) {
		CDBG("m5mo_sensor_open_init failed!\n");
		rc = -ENOMEM;
		goto init_done;
	}	
...
int m5mo_sensor_release(void)
{
	...
	CAM_DEBUG(" 1. CAM_8M_RST LOW ");
	gpio_set_value_cansleep(CAM_8M_RST, LOW);
	mdelay(3);

	////////////////////// test
	CAM_DEBUG(" 2. CAM_VGA_RST LOW ");
	gpio_set_value_cansleep(CAM_VGA_RST, LOW);
	//////////////////////

	//MLCK??????
	
	CAM_DEBUG("3. PMIC OFF ");
	cam_ldo_power_off( );
	mdelay(10);
