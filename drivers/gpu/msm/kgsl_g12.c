				...
				unsigned int msecs)
{
	int status = -EINVAL;
	long timeout = 0;
	struct kgsl_device *adreno_device = kgsl_get_device(0);

	if(adreno_device->state == KGSL_STATE_ACTIVE || adreno_device->state == KGSL_STATE_NAP)
	{
		timeout = wait_event_interruptible_timeout(
					device->wait_queue,
					kgsl_check_timestamp(device,
