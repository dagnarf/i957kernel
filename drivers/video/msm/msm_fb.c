...
#define MSM_FB_NUM	3
#endif
/*  Samsung flag, for the charging mode booting */
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	if (mfd->index == 0) {
		...
		sec_getlog_supply_fbinfo(fbram_phys, 800, 1280, 32, 2);
#endif
#ifdef CONFIG_MACH_P4_LTE
		sec_getlog_supply_fbinfo(fbram_phys, 1280, 800, 32, 2);
#endif
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	if(mfd->index == 0)
	{
		if(charging_mode_from_boot) {
			if (!load_565rle_image(CHARGING_IMAGE_FILE)) ;	/* Flip buffer */
				printk("[LCD] working charging mode...\n");
		}else {
