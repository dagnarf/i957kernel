...
#ifdef CONFIG_SAMSUNG_HDMI_DDC_I2C_SOLUTION	//Resolve ddc i2c fail
#include <linux/i2c.h>
#endif
...
#define hdmi_ddc_I2C_NAME "hdmi_ddc"
#define hdmi_ddc_hdcp_I2C_NAME "hdmi_ddc_hdcp"

struct hdmi_ddc_state {
	struct i2c_client *client;
};

extern struct i2c_driver hdmi_ddc_i2c_driver;
extern struct i2c_driver hdmi_ddc_hdcp_i2c_driver;

static const struct i2c_device_id hdmi_ddc_id[] = {
	{hdmi_ddc_I2C_NAME, 0 },
	{ }
};
static const struct i2c_device_id hdmi_ddc_hdcp_id[] = {
	{hdmi_ddc_hdcp_I2C_NAME, 0 },
	{ }
};

struct i2c_client *hdmi_ddc_i2c_client;
struct i2c_client *hdmi_ddc_hdcp_i2c_client;
...
static irqreturn_t hdmi_msm_isr(int irq, void *dev_id)
{
	...
	uint32 hpd_int_ctrl;
#ifndef CONFIG_SAMSUNG_HDMI_DDC_I2C_SOLUTION
	uint32 ddc_int_ctrl;
#endif /*CONFIG_SAMSUNG_HDMI_DDC_I2C_SOLUTION*/
...
#ifdef CONFIG_SAMSUNG_HDMI_DDC_I2C_SOLUTION
static u8 hdmi_ddc_i2c_read(struct i2c_client *client, u8 reg)
{
	u8 ret = i2c_smbus_read_byte_data(client, reg);
	if (ret < 0) {
		printk("[YHM] hdmi_ddc_i2c_i2c read fail\n");
		return -EIO;
	}
	return ret;
}

static int hdmi_ddc_i2c_write(struct i2c_client *client, u8 reg, u8 data)
{
	...
	const uint8 *data_buf, uint32 data_len, const char *what)
{
	...
	for(i=0; i<data_len; i++){
		ret = hdmi_ddc_i2c_write(hdmi_ddc_hdcp_i2c_client, offset+i, data_buf[i]);
		if(ret<0)
			return -EIO;
	}
	return 0;
	...
	uint32 data_len, int retry, const char *what, boolean no_align)
{
	int i=0;
	for(i=0; i<data_len; i++){
		data_buf[i] = hdmi_ddc_i2c_read(hdmi_ddc_hdcp_i2c_client, i+offset);
		if(data_buf[i]<0)
			return -EIO;			
	}
	return 0;
...
static int hdmi_msm_read_edid_block(int block, uint8 *edid_buf)
{
	...
	uint32 ndx;
		...
		if(block >= 2){
			for(i=0; i<128; i++){	
				/*Set Segment*/
				hdmi_ddc_i2c_write(hdmi_ddc_i2c_client, 0x60, 0x1);
				edid_buf[i] = hdmi_ddc_i2c_read(hdmi_ddc_i2c_client, i);
				}
		}else{
			for(i=0; i<128; i++)
				edid_buf[i] = hdmi_ddc_i2c_read(hdmi_ddc_i2c_client, i+block_size*block);
		}
...
#ifdef CONFIG_SAMSUNG_HDMI_DDC_I2C_SOLUTION
static int hdmi_ddc_i2c_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	struct hdmi_ddc_state *state;
	
	state = kzalloc(sizeof(struct hdmi_ddc_state), GFP_KERNEL);
	if (!state) {
	...
	pr_info("hdmi_ddc attach success!!!\n");

	hdmi_ddc_i2c_client = client;
	return 0;
}

static int hdmi_ddc_remove(struct i2c_client *client)
{
	struct hdmi_ddc_state *state = i2c_get_clientdata(client);
	kfree(state);
	return 0;
}

static int hdmi_ddc_hdcp_i2c_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	struct hdmi_ddc_state *state;
	...
	pr_info("hdmi_ddc_hdcp attach success!!!\n");

	hdmi_ddc_hdcp_i2c_client = client;

	return 0;

}

static int hdmi_ddc_hdcp_remove(struct i2c_client *client)
{
	struct hdmi_ddc_state *state = i2c_get_clientdata(client);
	kfree(state);
	return 0;
...
struct i2c_driver hdmi_ddc_i2c_driver = {
	.probe	= hdmi_ddc_i2c_probe,
	.remove	= hdmi_ddc_remove,
	.id_table	= hdmi_ddc_id,
	.driver = {
		.name = hdmi_ddc_I2C_NAME,
	},
};

struct i2c_driver hdmi_ddc_hdcp_i2c_driver = {
	.probe	= hdmi_ddc_hdcp_i2c_probe,
	.remove	= hdmi_ddc_hdcp_remove,
	.id_table	= hdmi_ddc_hdcp_id,
	.driver = {
		.name = hdmi_ddc_hdcp_I2C_NAME,
	},
};
#endif /*CONFIG_SAMSUNG_HDMI_DDC_I2C_SOLUTION*/
...
#ifdef CONFIG_SAMSUNG_HDMI_DDC_I2C_SOLUTION
	retval = i2c_add_driver(&hdmi_ddc_i2c_driver);
	if (retval)
		pr_err("[hdmi_ddc_i2c_driver] can't add i2c driver\n");
	else
		pr_info("[hdmi_ddc_i2c_driver] add i2c driver\n");
	retval = i2c_add_driver(&hdmi_ddc_hdcp_i2c_driver);
	if (retval)
		pr_err("[hdmi_ddc_hdcp_i2c_driver] can't add i2c driver\n");
	else
		pr_info("[hdmi_ddc_hdcp_i2c_driver] add i2c driver\n");
#endif /*CONFIG_SAMSUNG_HDMI_DDC_I2C_SOLUTION*/
...
static int __init hdmi_msm_init(void)
{
	...
	external_common_state->video_resolution = HDMI_VFRMT_1920x1080p30_16_9;		//hmyun0621
