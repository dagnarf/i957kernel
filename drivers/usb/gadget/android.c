...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	if (dev->requested_usb_mode == USBSTATUS_MDM)
		return;
	...
	if(enable) {
		...
		if (!strcmp(f->name, "rmnet_sdio")) {
			ret = set_product(dev, USBSTATUS_MDM);
			if (ret != -1)
				dev->current_usb_mode = USBSTATUS_MDM;
		}
	}
...
void samsung_enable_function(int mode)
{
	...
	switch(mode) {
		...
		case USBSTATUS_ACCESSORY: /* do not save usb mode */
			printk("mode = USBSTATUS_ACCESSORY (0x%x)\n", mode);
			ret = set_product(dev, USBSTATUS_ACCESSORY);
			break;
#endif				
		case USBSTATUS_MDM:
...
static ssize_t UsbMenuSel_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(a_dev->cdev) {
		switch(a_dev->requested_usb_mode) {
			...
			case USBSTATUS_MDM:
				return sprintf(buf, "[UsbMenuSel] MDM TEST\n");				
		}
	}
	return sprintf(buf, "%d\n", value);
}
...
static ssize_t UsbMenuSel_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	switch(value) {
		...
		case 3:
			...
			break;
		case 4:
			samsung_enable_function(USBSTATUS_MDM);
			break;
		case 5:
			samsung_enable_function(USBSTATUS_ADB);
			break;
