...
void usb_composite_force_reset(struct usb_composite_dev *cdev)
{
	...
	if (cdev && cdev->gadget &&
		...
		printk("[%s] state: %d\n", __func__, cdev->sw_usbreset.state);
	} else {
		spin_unlock_irqrestore(&cdev->lock, flags);
	}
}
...
static void composite_disconnect(struct usb_gadget *gadget)
{
	 ...
	 */
	printk("[%s] config: 0x%p \n",__func__, (void*)(cdev->config));
...
composite_switch_work(struct work_struct *data)
{
	...
	printk("[%s] config: 0x%p, connected: %d, sw_connected.state: %d\n",
	__func__, (void*)config, cdev->connected, cdev->sw_connected.state );
...
#endif
	if (config) {
		printk("[%s] config: 0x%p, bConfigurationValue :%d\n",
		__func__, (void*)config, config->bConfigurationValue );
