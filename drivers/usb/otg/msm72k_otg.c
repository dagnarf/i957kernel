...
static void msm_otg_start_peripheral(struct otg_transceiver *xceiv, int on)
{
	...
	if (on) {
		...
		if (!dev->pdata->vbus_blocked)
			usb_gadget_vbus_connect(xceiv->gadget);
		else
			pr_info("usb vbus is blocked!!\n");
	} else {
...
reset_link:
	...
	set_se1_gating(dev);
	ulpi_write(dev, ulpi_read(dev,ULPI_CONFIG_REG2) | ULPI_AMPLITUDE_MAX, ULPI_CONFIG_REG2); //For USB eye-diagram pattern
...
#define NOTIFY_DRIVER_NAME "usb_otg"
	...
	if (dev->pdata->otg_en)
		the_msm_otg->ndev.set_booster = dev->pdata->otg_en;
